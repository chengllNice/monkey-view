<template>
    <div :class="[
        ...
        <div :class="[`${classPrefix}__title`]" ref="reference" :style="titleStyle" @click.stop="handlerClick">
            <slot name="title">
                <Icon v-if="icon" :type="icon" :class="icon"></Icon>
                <span>{{name}}</span>
            </slot>
            <Icon v-if="!hideDropIcon" :type="openedIcon" :class="[`${classPrefix}__title-slide-icon`]"></Icon>
        </div>
